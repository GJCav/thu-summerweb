...
let enableTimeDelta = false;

function incTimeDelta(){
  if(enableTimeDelta){
    timeDelta.value += 1;
    setTimeout(incTimeDelta, 1000);
  }
}
...
onMounted(async () => {
  ...
  if(!profile.value.login) navigateTo("/")
  else{
    timeDelta.value = 0
    enableTimeDelta = true
    incTimeDelta()
  }
})
